...
class RGBAdapter(nn.Module):
    ...
    # @staticmethod
    def _normalization(self, x, mask=None):
        ...
        x = x.sub(torch.tensor([123.675, 116.280, 103.530]).view(1, 3, 1, 1).to(self.device)).div(
            torch.tensor([58.395, 57.120, 57.375]).view(1, 3, 1, 1).to(self.device))
        return x
