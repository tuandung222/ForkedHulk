...
class Hulk_Decoder(nn.Module):
    ...
    def forward_recons(self, x):
        ...
        positional_embeddings = positional_embeddings.permute(1, 0, 2) # num_tokens_1, bs, dim
        context_with_mask = context_with_mask.permute(1, 0, 2) # num_tokens_2, bs, dim
        
        
        diff_len = len(positional_embeddings) - len(context_with_mask)
        

        if diff_len > 0:
            # positional_embeddings = positional_embeddings[:len(context_with_mask)]
            # padding the context_with_mask
            context_with_mask = torch.cat([context_with_mask, torch.zeros(diff_len, batch_size, self.hidden_dim).to(context_with_mask.device)], dim=0)
        ...
        if self.adding_per_layer_pe:
            ...
            query_pos = None
            print("context_with_mask shape: ", context_with_mask.shape)
