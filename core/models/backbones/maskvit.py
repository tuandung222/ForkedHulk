...
class maskViT(nn.Module):
        ...
        for i in range(depth):
            ...
            if self.lms_checkpoint_train == 'fairscale':
                try:
                    ...
                    if i==0:
                        print(f'[Rank {self.rank}] fairscale checkpoint success')
                except:
                    if i==0:
                        print(f'[Rank {self.rank}] fairscale checkpoint failed, use naive block')
                    pass
    ...
    def generate_input_info(self, input_task_tokens, ):
        ...
        for domain, tensor in input_task_tokens.items():
            ...
            input_info['tasks'][domain] = {
                ...
                'start_idx': u,
                'end_idx': u + num_tokens,
            }
            u = u + num_tokens
        # input_info['image_size'] = image_size
        input_info['num_task_tokens'] = u
    ...
    def generate_random_masks(self,
        ...
        # Do not use Dirichlet now
        # Just uniform sampling
        task_sampling_dist = torch.ones(B, 2, device=device, requires_grad=False)
